...
def display_feed(request):
    ...
    reviews = Review.objects.filter (Q(user_id__in=all_connected_users) | Q(ticket__user_id=user.id))
...
def unfollow_user(request, user_id):
    ...
    messages.success(request,f"Vous êtes maintenant désabonné à {user_to_unfollow.username}")
    return redirect('display_followers')
...
def search_user_to_follow(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            elif user_to_follow != request.user and not UserFollows.objects.filter(follower=request.user, followed=user_to_follow).exists():
                UserFollows.objects.create(follower=request.user, followed=user_to_follow)
                messages.success(request,f"Vous êtes maintenant abonné à {user_to_follow.username}")
